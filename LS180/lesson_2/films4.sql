...
-- list all genres with existing movies
SELECT DISTINCT(genre)
FROM films
WHERE title IS NOT NULL;

-- list all genres with existing movies without using distinct
...
-- average duration during 10 years
SELECT year / 10 * 10 AS decade, ROUND(AVG(duration))
FROM films
GROUP BY decade
ORDER BY decade;

-- find all films with director first name 'John'
SELECT *
FROM films
...
-- return title group by decade and genre
SELECT year / 10 * 10 AS decade, genre, STRING_AGG(title, ', ') AS title
FROM films
GROUP BY decade, genre ORDER BY decade, genre;

-- return total duration group by genre
